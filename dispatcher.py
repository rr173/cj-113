...
class DispatchEngine:
    ...
    def execute(self, now: datetime = None) -> DispatchDecision:
        ...
        if island_grid_blocked:
            original_grid_import_kw = grid_import_kw
            original_grid_export_kw = grid_export_kw

            if original_grid_import_kw > 0.001:
                grid_import_cost_kwh = original_grid_import_kw * time_interval_hours
                total_cost -= original_grid_import_kw * time_interval_hours * grid_buy_price
                load_grid_import_kwh = max(0.0, load_grid_import_kwh - grid_import_cost_kwh)

                def _is_grid_import_note(n):
                    return (n.startswith("外购电 ")
                            or n.startswith("[碳预警] 外购电 ")
                            or n.startswith("[甩负荷容忍带] "))

                notes[:] = [n for n in notes if not _is_grid_import_note(n)]

            if original_grid_export_kw > 0.001:
                total_cost += original_grid_export_kw * time_interval_hours * feed_in_price

                def _is_grid_export_note(n):
                    return n.startswith("余电上网 ")

                notes[:] = [n for n in notes if not _is_grid_export_note(n)]

            if original_grid_import_kw > 0.001:
                island_remaining = original_grid_import_kw
                grid_import_kw = 0.0
                ...
                if diesel_cap["can_run"]:
                    ...
                    if diesel_fill > 0:
                        ...
                        extra_diesel_kwh = diesel_fill * time_interval_hours
                        self.state.diesel_state[ds_id_island].total_generated_kwh += extra_diesel_kwh
                        self.state.stats.total_diesel_generated_kwh += extra_diesel_kwh
            ...
            if original_grid_export_kw > 0.001:
                surplus_island = original_grid_export_kw
                grid_export_kw = 0.0
