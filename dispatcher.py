...
class DispatchEngine:
    ...
    def _detect_fault_sources_with_plan(self):
        fault_with_plan = []
        for sid, r in self.state.pv_reports.items():
            if not r.available:
                plan = self.state.get_backup_plan_for_source(f"pv:{sid}")
                if plan is not None:
                    fault_with_plan.append(("pv", sid, plan))
        for sid, r in self.state.wt_reports.items():
            if not r.available:
                plan = self.state.get_backup_plan_for_source(f"wt:{sid}")
                if plan is not None:
                    fault_with_plan.append(("wt", sid, plan))
        for sid, r in self.state.diesel_reports.items():
            if not r.available:
                plan = self.state.get_backup_plan_for_source(f"diesel:{sid}")
                if plan is not None:
                    fault_with_plan.append(("diesel", sid, plan))
        return fault_with_plan
    ...
    def execute(self, now: datetime = None) -> DispatchDecision:
        ...
        fault_with_plan = self._detect_fault_sources_with_plan()
        force_battery_priority = len(fault_with_plan) > 0
        ...
        for ftype, fsid, fplan in fault_with_plan:
            notes.append(f"按预案执行：{ftype}:{fsid} 掉线，参考预案 {fplan.plan_id}")
        ...
        if remaining_load > 0:
            ...
            if discharge_kw > 0:
                ...
                if force_battery_priority:
                    notes.append(f"[预案模式] 电池优先放电 {discharge_kw:.2f}kW")
                else:
            ...
            if remaining_load > 0:
                if force_battery_priority:
                    use_grid = False
                    notes.append("[预案模式] 电池放电优先，跳过买电优先判断")
                else:
                ...
                if use_grid:
                    ...
                    if diesel_cap["can_run"]:
                        ...
                        if force_battery_priority:
                            ...
                            remaining_load = 0
                            total_cost += grid_import_kw * time_interval_hours * grid_buy_price
                            notes.append(f"[预案模式] 柴油机不可用，外购电 {grid_import_kw:.2f}kW 兜底")
                        else:
